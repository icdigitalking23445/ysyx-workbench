...
#define immJ() do { uint32_t raw_imm = \
      (BITS(i, 31, 31) << 20) | \
      (BITS(i, 19, 12) << 12) | \
      (BITS(i, 20, 20) << 11) | \
      (BITS(i, 30, 21) << 1); \
...
#define immB()do { uint32_t raw_imm = \
      (BITS(i, 31, 31) << 12) |\
      (BITS(i, 7, 7) << 11)   |\
      (BITS(i, 30, 25) << 5)  |\
      (BITS(i, 11, 8) << 1 ); \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra   , R, R(rd) = SEXT((int32_t)src1 >> BITS(src2, 4, 0), 32));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or    , R, R(rd) = src1 | src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and   , R, R(rd) = src1 & src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul   , R, R(rd) = (int64_t)(int32_t)src1 * (int64_t)(int32_t)src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem   , R, R(rd) = (int64_t)(int32_t)src1 % (int64_t)(int32_t)src2);
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", lwu    , I, R(rd) = SEXT(Mr(src1 + imm, 4),32));
  //INSTPAT("??????? ????? ????? 000 ????? 00000 11", addiw  , I, R(rd) = SEXT((int32_t)src1 + imm, 32));
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli,    I, R(rd) = src1 << BITS(imm, 4, 0));
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1 >> BITS(imm, 4, 0));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = SEXT((int32_t)src1 >> BITS(imm, 4, 0), 32));
  //U-type
